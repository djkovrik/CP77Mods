...
- Every `@wrapMethod` should call `wrappedMethod(...)` so the original method and other wrappers in the chain still run. Use the exact original overload arguments.
...
- Prefer existing game helpers and Codeware helpers over new utility code when they already solve the problem.
...
When working with `inkVirtualListController`, `inkVirtualGridController`, or other virtual compound controllers, check vanilla examples first:

- `<GAME_SOURCES_ROOT>\core\ui\scriptableDataSource.swift` for `ScriptableDataView`, `Filter()`, `EnableSorting()`, `DisableSorting()`, `Sort()`, `FilterItem(...)`, and `SortItem(...)`.
- `<GAME_SOURCES_ROOT>\orphans.swift` for `inkVirtualCompoundController.SetSource(...)`, `SetClassifier(...)`, `ScriptableDataSource.Reset(...)`, `AppendItem(...)`, `RemoveItem(...)`, `GetArray()`, `GetArraySize()`, `Size()`, and `GetItem(...)`.
- `<GAME_SOURCES_ROOT>\cyberpunk\UI\fullscreen\backpack\backpack_main.swift` for the main inventory-style pattern with `BackpackDataView`, `ItemDisplayTemplateClassifier`, and optional `ItemPositionProvider`.
- `<GAME_SOURCES_ROOT>\cyberpunk\UI\common\virtualNestedListController.swift`, `cyberpunk\UI\fullscreen\quest_log\questLog.swift`, `cyberpunk\UI\fullscreen\codex\codexVirtualControllers.swift`, and `cyberpunk\UI\fullscreen\codex\codexUtils.swift` for nested virtual lists and grouped/header rows.

Installed mod examples worth checking before adding a new virtual list:

- `RevisedBackpack\RevisedBackpackController.reds` and `RevisedBackpack\RevisedBackpackDataView.reds` for a full custom inventory list with wrappers, selection state, filtering, sorting, and visible-controller refresh.
- `VirtualCarDealer\autofixer\AutofixerVirtualController.reds`, `VirtualCarDealer\autofixer\AutofixerItemDataView.reds`, and `VirtualCarDealer\core\CarDealer-Classes.reds` for a simple grid sorted by title.
- `virtual-atelier-full\virtual-store\VirtualStoreController.reds`, `virtual-atelier-full\virtual-store\VirtualStoreDataView.reds`, `virtual-atelier-full\virtual-store\VirtualStoreTemplateClassifier.reds`, `virtual-atelier-full\stores-list\AtelierStoresListController.reds`, and `virtual-atelier-full\stores-list\AtelierStoresDataView.reds` for store/item grids, search, category filters, and custom template selection.
- `MetroPocketGuide\widget\TrackedRouteListController.reds`, `MetroPocketGuide\widget\TrackedRouteDataView.reds`, and `MetroPocketGuide\widget\TrackedRouteItemClassifier.reds` for a small route list with multiple row templates.

The usual controller setup is:

1. Get the virtual list/grid controller from an ink widget path or ref.
2. Create `ref<ScriptableDataSource>`.
3. Create a `ScriptableDataView` subclass when filtering or sorting is needed.
4. Call `dataView.SetSource(dataSource)`.
5. Create an `inkVirtualItemTemplateClassifier` subclass when rows use multiple item templates.
6. Call `virtualController.SetClassifier(classifier)` before or near `virtualController.SetSource(dataView)`.
7. If the vanilla widget expects positions, create and set the matching provider, as `BackpackMainGameController` does with `ItemPositionProvider`.
8. Populate an `array<ref<IScriptable>>`, call `dataSource.Reset(items)`, then call the view's refresh/sort routine if the list should immediately reorder or refilter.

Keep responsibilities separated:

- `ScriptableDataSource` owns the raw unfiltered items. Use `Reset(items)` for full rebuilds, `AppendItem(...)`/`RemoveItem(...)` for incremental changes, and `GetArray()` when logic must inspect all source items regardless of current filters.
- `ScriptableDataView` owns presentation state: active filter/category/search query, sort mode, and `FilterItem(...)`/`SortItem(...)`. Use `Size()` and `GetItem(index)` on the view when logic needs the current visible/filtered order.
- The controller owns widget wiring, events, selection/hover state, scroll position, and conversion from domain data into `ref<IScriptable>` row objects.
- The template classifier only maps item data to a template index via `ClassifyItem(data: Variant) -> Uint32`. Convert with `FromVariant<ref<IScriptable>>(data)` and downcast defensively. Return `0u` as the safe default.
- Row item data should be `IScriptable` wrapper/data classes (`WrappedInventoryItemData`, `VendorInventoryItemData`, mod-specific item wrappers, route points, shop records), not raw structs.

Sorting and filtering patterns:

- A common `UpdateView()` implementation temporarily enables sorting, calls `Sort()`, then disables sorting. This appears in RevisedBackpack, VirtualCarDealer, VirtualAtelier, and MetroPocketGuide.
- If a DataView may already have sorting enabled, follow vanilla `BackpackDataView.SetSortMode(...)`: remember `IsSortingEnabled()`, enable/sort/disable only when needed, otherwise just call `Sort()`.
- Filter setters usually update DataView state and call `Filter()`. Sort setters usually update state and call `Sort()` or an `UpdateView()` helper.
- For inventory-derived DataViews, prefer extending `BackpackDataView` and overriding extension points such as `DerivedFilterItem(...)`, `PreSortingInjection(...)`, or `PreFilterInjection(...)` instead of reimplementing the full category/sort logic.
- After search or category changes in inventory/vendor-like lists, it is common to call `Filter()`, briefly enable sorting, restore filter/sort mode, then disable sorting, matching VirtualAtelier and stash/backpack search mods.

Lifecycle and compatibility:

- On detach/uninitialize, detach the virtual controller and classifier with `SetSource(null)` and `SetClassifier(null)`, then call `dataView.SetSource(null)` before nulling fields. Also clear any position/provider object if one was set.
- Keep `ref<>` for DataSource/DataView/classifier fields and `wref<>` for widget/controller references unless nearby code uses stronger ownership.
- Preserve module visibility: DataView/classifier classes used across files or modules need `public` and matching imports.
- Do not refresh every row widget by rebuilding the whole source when only visible row state changed. RevisedBackpack iterates `GetRootCompoundWidget()` children and calls each item controller's `RefreshView()` for visible-item updates.
- When selection logic must respect the visible filtered list, iterate the DataView with `Size()` and `GetItem(index)`. When it must affect all backing items, iterate `dataSource.GetArray()`.
...
- If a compiler or game log is available in the local setup, use it to verify redscript errors before reporting completion.
