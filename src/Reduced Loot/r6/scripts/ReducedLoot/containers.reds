...
private func EvaluateLoot() -> Void {
  ...
  if transactionSystem.GetItemList(this, items) {
    while i < ArraySize(items) {
      ...
      if RL_Checker.CanLootThis(item, RL_LootSource.Container) || this.IsQuest() || item.HasTag(n"Quest") || shouldKeepForId || shouldKeepForQuest || Equals(item.GetItemType(), gamedataItemType.Gen_Keycard) {
