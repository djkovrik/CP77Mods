...
private final func FillVirtualStock() -> Void {
  ...
  while virtualItemIndex < ArraySize(storeItems) {
    ...
    AtelierDebug(s"Store item: \(ToString(storeItems[virtualItemIndex]))", this.m_config);
    ...
    AtelierDebug(s"   Dynamic tags: \(ToString(itemData.GetDynamicTags()))", this.m_config);
    AtelierDebug(s"   VirtPrice: \(ToString(stockItem.price))", this.m_config);
    if (RoundF(stockItem.price) == 0) {
      stockItem.price = Cast<Float>(AtelierItemsHelper.ScaleItemPrice(this.m_player, vendorObject, itemId, stockItem.quality) * stockItem.quantity);
     };
    AtelierDebug(s"   CalcPrice: \(ToString(stockItem.price))", this.m_config);
...
private final func ScaleStockItems() -> Void {
  ...
  while i < ArraySize(this.m_virtualStock) {
    ...
    if !itemRecord.IsSingleInstance() && !itemData.HasTag(n"Cyberware") {
      AtelierItemsHelper.ScaleItem(this.m_player, this.m_virtualStock[i].itemData, this.m_virtualStock[i].quality);
    };
    i += 1;
  };
}
...
private func PopulateVirtualShop() -> Void {
  ...
  AtelierDebug(s"Resulting list size: \(vendorInventorySize)", this.m_config);
