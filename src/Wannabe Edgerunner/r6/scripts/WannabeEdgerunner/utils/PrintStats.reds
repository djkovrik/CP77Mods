...
public abstract class EdgerunnerStats {
  public static func Print(system: ref<EdgerunningSystem>, config: ref<EdgerunningConfig>) -> Void {
	...
	let overclock: Int32 = config.overclockUsageCost;
    ...
    let ol1 = upper / Cast<Int32>(Cast<Float>(overclock) * config.qualityMultiplierLegendary);
    let oe1 = upper / Cast<Int32>(Cast<Float>(overclock) * config.qualityMultiplierEpic);
    let or1 = upper / Cast<Int32>(Cast<Float>(overclock) * config.qualityMultiplierRare);
    let ou1 = upper / Cast<Int32>(Cast<Float>(overclock) * config.qualityMultiplierUncommon);
    let oc1 = upper / Cast<Int32>(Cast<Float>(overclock) * config.qualityMultiplierCommon);
    ...
    E(s"Overclock usages before psycho: Legendary \(ol1), Epic \(oe1), Rare \(or1), Uncommon \(ou1), Common \(oc1)");
