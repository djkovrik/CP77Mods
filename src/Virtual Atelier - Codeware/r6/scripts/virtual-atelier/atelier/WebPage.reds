...
public func PopulatePaginationControls() -> Void {
  ...
  let prevButton = AtelierTextButton.Create(n"ButtonPrev", VirtualAtelierText.PaginationPrev(), 68, n"MainColors.Blue", n"MainColors.ActiveBlue", new inkMargin(0.0, 0.0, 0.0, 0.0), true);
  prevButton.RegisterToCallback(n"OnClick", this, n"OnClickPagination");
  prevButton.Reparent(panel);
  this.navigationButtonPrev = prevButton;

  let label = AtelierTextButton.Create(n"NavLabel", this.GetPaginationLabel(), 70, n"MainColors.Red", n"", new inkMargin(40.0, 0.0, 40.0, 0.0), false);
  label.Reparent(panel);
  this.navigationLabel = label;

  let nextButton = AtelierTextButton.Create(n"ButtonNext", VirtualAtelierText.PaginationNext(), 68, n"MainColors.Blue", n"MainColors.ActiveBlue", new inkMargin(0.0, 0.0, 0.0, 0.0), true);
  nextButton.RegisterToCallback(n"OnClick", this, n"OnClickPagination");
  nextButton.Reparent(panel);
  this.navigationButtonNext = nextButton;
